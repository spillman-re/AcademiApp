...
export class GrupoService {
    ...
    async getGrupo(id: number) {
        ...
        if (result.recordset.length === 0) {
            throw new NotFoundException(
                `El grupo con id ${id} no fue encontrado.`
            );
        }
    ...
    async createGrupo(grupo: CreateGrupoDto) {
        ...
        const result = await pool.request()
            ...
            .input('fecha_inicio', grupo.fecha_inicio)
            .query(`
                INSERT INTO grupo (
                    id_curso,
                    nombre_grupo,
    ...
    async deleteGrupo(id: number) {
        ...
        if (grupo.estado !== 'ACTIVO') {
            throw new BadRequestException(
                'El grupo ya no se encuentra activo.'
            );
        }
    ...
    private async actualizarEstadoCurso(idCurso: number) {
        ...
        const nuevoEstado = hayFinalizados
            ? 'FINALIZADO'
            : 'CANCELADO';
